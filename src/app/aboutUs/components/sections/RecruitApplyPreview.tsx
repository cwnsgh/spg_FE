...
function schoolEndCircleKey(
  stored: string
): (typeof SCHOOL_END_DISPLAY)[number] | "" {
  const t = stored.trim();
  if (!t) return "";
  if (t === "졸업") return "졸업";
  if (t === "중퇴") return "중퇴";
    ...
    <span className={styles.spgChoiceRow}>
      {SCHOOL_END_DISPLAY.map((label) => (
        <span
          key={label}
          className={
            label === key ? styles.spgCircleChoiceOn : styles.spgCircleChoiceOff
          }
        >
          {label}
        </span>
...
function UnionCircles({ value }: { value: string }) {
  ...
  return (
    <span className={styles.spgUnionPair}>
      <span
        className={
          k === "유" ? styles.spgCircleChoiceOn : styles.spgCircleChoiceOff
        }
      >
        유
      </span>
      <span
        className={
          k === "무" ? styles.spgCircleChoiceOn : styles.spgCircleChoiceOff
        }
      >
        무
      </span>
...
export interface RecruitApplyPreviewProps {
  ...
  step1Attachments?: Partial<
    Record<
      "fgrade" | "fscore" | "fcerti" | "fcareer",
      RecruitStep1AttachmentRow[]
    >
  >;
}
...
export default function RecruitApplyPreview({
  ...
  if (Array.isArray(armyRaw) && armyRaw[0] && typeof armyRaw[0] === "object") {
    ...
    armyRaw &&
    typeof armyRaw === "object" &&
    !Array.isArray(armyRaw)
  ) {
  ...
  const openAttachmentPreview = (
    sectionTitle: string,
    file: RecruitStep1AttachmentRow
  ) => {
  ...
  const textOrEmpty = (s: string) =>
    s.trim() ? (
      <span className={styles.pre}>{s}</span>
    ) : (
    ...
    <Fragment>
      <div className={styles.wrap}>
        ...
        <div className={styles.applicationSheet}>
          <div className={styles.spgAppHeader}>
            ...
            <section className={styles.spgSection}>
              ...
              {STEP1_ATTACHMENT_LABELS.map(({ key, label: blockTitle }) => {
                ...
                return (
                  <div
                    key={key}
                    className={`${styles.attachBlock} ${styles.spgAttachBlock}`}
                  >
                    ...
                    {list.length === 0 ? (
                      ...
                      <ul className={styles.attachList}>
                        {list.map((f, i) => {
                          const fileName =
                            norm(f.pf_source) || norm(f.pf_file) || "(파일)";
                            ...
                            <li key={f.pf_id ?? `${key}-${i}`}>
                              {href ? (
                                <button
                                  ...
                                  onClick={() =>
                                    openAttachmentPreview(blockTitle, f)
                                  }
                                >
                                  <span className={styles.attachPreviewName}>
                                    {fileName}
                                  </span>
                                  <span className={styles.attachPreviewAction}>
                                    미리보기
                                  </span>
                                </button>
                              ) : (
          ...
          <section className={styles.spgSection}>
            ...
            {schoolRows.length === 0 ? (
              ...
              <table
                className={`${styles.spgTable} ${styles.spgTableCentered}`}
              >
                <thead>
                  <tr>
                    <th>기간</th>
                ...
                <tbody>
                  {schoolRows.map((r, i) => {
                    ...
                    const nameIdx = schoolRows.findIndex((x) =>
                    ...
                    return (
                      <tr key={i}>
                        ...
                        <td className={styles.spgTdName}>
                          {showRecent ? (
                            <span
                              className={styles.spgRecentBadge}
                              title="최종학력"
                            >
                              최근
                            </span>
          ...
          <section className={styles.spgSection}>
            ...
            {careerRows.length === 0 ? (
              ...
              <div className={styles.spgHorizScroll}>
                <table
                  className={`${styles.spgTable} ${styles.spgTableCareer} ${styles.spgTableCentered}`}
                >
                  <thead>
                    <tr>
                      <th>업체명</th>
                  ...
                  <tbody>
                    {careerRows.map((r, i) => {
                      const o = rowObj(r);
                      const firstCareerIdx = careerRows.findIndex((x) =>
                        norm(rowObj(x).name).trim()
                      );
                      const showRecent =
                        norm(o.name).trim() &&
                        firstCareerIdx === i &&
                        careerRows.some((x) => norm(rowObj(x).name).trim());
                      return (
                        <tr key={i}>
                          <td className={styles.spgTdName}>
                            {showRecent ? (
                          ...
                          <td>{norm(o.part)}</td>
                          <td className={styles.spgCareerCont}>
                            {norm(o.cont)}
                          </td>
                          <td>{norm(o.salary)}</td>
        ...
        <section className={`${styles.section} ${styles.docSectionProfile}`}>
          <div className={styles.docSheetHeader}>
            <div className={styles.docSheetHeaderLeft}>
              {norm(data.re_name)}
              {norm(data.re_sex) ? `(${norm(data.re_sex)})` : ""}{" "}
              {norm(data.re_hp)}
            </div>
            ...
            return (
              <div key={k} className={styles.profileQBlock}>
                ...
                <div className={styles.profileQBody}>
                  {val ? (
                    <p className={styles.pre}>{val}</p>
                  ) : (
                    <p className={styles.empty}>(미입력)</p>
                  )}
                </div>
              </div>
            );
        ...
        <section className={`${styles.section} ${styles.docSectionCareer}`}>
          ...
          {historyRows.length === 0 ? (
            ...
            historyRows.map((r, i) => {
              ...
              const firstNamedIdx = historyRows.findIndex((x) =>
                norm(rowObj(x).name).trim()
              );
                    ...
                    <tbody>
                      <tr>
                        <th>
                          ...
                          {showRecent ? (
                            <span
                              className={styles.historyRecentBadge}
                              title="가장 최근 경력"
                            >
                              최근
                            </span>
                      ...
                      <tr>
                        <th>근무기간</th>
                        <td colSpan={3}>
                          {norm(o.sdate)} ~ {norm(o.edate)} ( {norm(o.year)}년{" "}
                          {norm(o.month)}개월 )
                        </td>
                      </tr>
                      <tr>
                        <th className={styles.historyDetailTh}>
                          세부경력 소개
                        </th>
                        <td colSpan={3} className={styles.historyDetailTd}>
                          {norm(o.cont) ? (
                            <span className={styles.pre}>{norm(o.cont)}</span>
                          ) : (
                            ""
                          )}
                        </td>
        ...
        >
          ...
          >
            <div className={styles.filePreviewHeader}>
              <div>
                <p className={styles.filePreviewEyebrow}>
